...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		return pow((_x - m_center).squaredNorm() + (m_radius * m_radius) - (m_a * m_a), 2) - 4 * m_radius * m_radius * (((_x - m_center)[0] * (_x - m_center)[0]) + ((_x - m_center)[1] * (_x - m_center)[1]));
	}
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		for (int i = 0; i < m_numCenters; ++i)
		{
			result += m_coefficents[i] * pow((m_funcSamp.m_pos[i] - _x).norm(), 3);
		}
		result += Eigen::Vector3d(m_coefficents[m_numCenters], m_coefficents[m_numCenters + 1], m_coefficents[m_numCenters + 2]).dot(_x) + m_coefficents[m_numCenters + 3];
		return result;
	}
...
#define phi(i,j) EvalBasis((m_funcSamp.m_pos[i]-m_funcSamp.m_pos[j]).norm())
	...
	void BuildSystem()
	{
		...
		for (int i = 0; i < 2 * m_numCenters; ++i)
		{
			for (int j = m_numCenters; j < m_numCenters + 3; ++j)
			{
				A(i, j) = m_funcSamp.m_pos[i][j];
			}
		}
